...
class General(commands.Cog, name="general"):
    ...
    async def botinfo(self, context: Context) -> None:
        ...
        """
        embed = discord.Embed(
            description="이 디스코드 봇은 [Krypton's](https://krypton.ninja) 템플릿을 사용하여 개발하였습니다.",
            color=0xBEBEFE,
        )
        embed.set_author(name="봇 정보")
        embed.add_field(name="개발자 정보:", value="Sullung#7780", inline=True)
        embed.add_field(
            name="Python version:", value=f"{platform.python_version()}", inline=True
        )
        embed.add_field(
            name="명령어 사용 방법 :",
            value=f"'{self.bot.config['prefix']}'를 사용해 주세요",
            inline=False,
        )
        embed.set_footer(text=f"마지막 패치 날짜 : 2024-12-25")
        await context.send(embed=embed)
    ...
    async def bitcoin(self, context: Context) -> None:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                "https://crix-api-cdn.upbit.com/v1/crix/trades/days?code=CRIX.UPBIT.KRW-BTC&count=2&convertingPriceUnit=KRW",
                headers={
                ...
                if request.status == 200:
                    ...
                    embed = discord.Embed(
                        title="비트코인 가격",
                        description=f"UPbit 기준 현재 비트코인 가격 : **{format(int(data[0]['tradePrice']), ',d')}원**",
                        color=0xBEBEFE,
                    )
                else:
                    embed = discord.Embed(
                        title="Error!",
                        description="API 문제가 발생했습니다. 관리자에게 문의해주세요.",
                        color=0xE02B2B,
                    )
                await context.send(embed=embed)
